...
class CleanupOrphanedChunks extends Command
{
    ...
    public function handle()
    {
        ...
        // Step 3: Clean up expired upload locks
    ...
    protected function cleanupExpiredCache()
    {
        ...
        // Delete expired cache entries (upload status and locks)
        $deleted = DB::table($table)
            ->where(function($query) {
                $query->where('key', 'like', Cache::getPrefix() . 'upload_user_%')
                      ->orWhere('key', 'like', Cache::getPrefix() . 'upload_lock_user_%');
            })
            ...
            foreach ($directories as $dir) {
                ...
                if (count($parts) >= 3 && $parts[0] === 'chunks' && str_starts_with($parts[1], 'user_')) {
                    ...
                    try {
                        ...
                        // If modified in last 30 minutes, assume still in progress
                        if ($dirTime && $dirTime > now()->subMinutes(30)->timestamp) {
                            $isRecent = true;
                        }
                    } catch (\Exception $e) {
     ...
     */
    protected function cleanupExpiredLocks()
    {
        $cacheDriver = config('cache.default');
        ...
        $deleted = DB::table($table)
            ->where('key', 'like', Cache::getPrefix() . 'upload_lock_user_%')
            ->where('expiration', '<', $now)
            ->delete();
        
        if ($deleted > 0) {
            $this->info("Cleaned up {$deleted} expired upload locks");
        }
    }
}
