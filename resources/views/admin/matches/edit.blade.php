...
@section('title', __('admin.edit_match') . ' - Eye Pro')
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="mb-6">
        <a href="{{ route('matches.show', $match->id ?? 1) }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors">
            ...
            <span class="font-medium">{{ __('admin.back_to_match') }}</span>
        </a>
    </div>
    ...
    <div class="relative overflow-hidden rounded-2xl p-6 shadow-lg border border-blue-200 mb-6" style="background: linear-gradient(135deg, #60a5fa 0%, #818cf8 100%);">
        ...
        <div class="relative z-10">
            <h1 class="text-2xl font-bold text-white mb-1">{{ __('admin.edit_match') }}</h1>
            <p class="text-sm text-blue-50 font-medium">{{ __('admin.update_match_info') }}</p>
        </div>
    </div>
        ...
        <form action="{{ route('matches.update', $match->id ?? 1) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                <label for="match_name" class="block text-sm font-semibold text-gray-700 mb-2">
                    ...
                    {{ __('admin.match_name_required') }} <span class="text-red-500">*</span>
                </label>
                <input 
                    type="text" 
                    id="match_name" 
                    ...
                    required
                    placeholder="{{ __('admin.enter_match_name') }}"
                >
                @error('match_name')
            ...
            <div>
                <label for="description" class="block text-sm font-semibold text-gray-700 mb-2">
                    ...
                    {{ __('admin.description_optional') }} <span class="text-gray-400 text-xs font-normal">({{ __('admin.other') }})</span>
                </label>
                <textarea 
                    id="description" 
                    name="description" 
            ...
            <div>
                <label for="tags" class="block text-sm font-semibold text-gray-700 mb-2">
                    ...
                    {{ __('admin.tags_label') }} <span class="text-gray-400 text-xs font-normal">{{ __('admin.optional_label') }}</span>
                </label>
                <input 
                    type="text" 
                    id="tags" 
